...
const KanbanBoard = () => {
    ...
    function moveCards_SameContainer(active, over) {
        ...
        const clonedCard = {...updatedContainers[active.parentIdx].cards[activeCardIdx]};
        ...
        updatedContainers[active.parentIdx].cards.forEach((card, idx) => card.idx = idx);
    ...
    function moveCards_DifferentContainer(active,over) {
        ...
        const updatedContainers = [...containers];
        const clonedCard = {...updatedContainers[active.parentIdx].cards[active.idx]};
        updatedContainers[active.parentIdx].cards.splice(active.idx, 1);
        if(overCardIdx >= 0 && overCardIdx < updatedContainers[overContainerIdx].cards.length-1) { 
            ...
            clonedCard.idx = updatedContainers[overContainerIdx].cards.push(clonedCard)-1;
        }
    ...
    function handleDragOver({active, over}) { //Different container actions
        if(!over) {
            console.log("no over");
            return;
        }
